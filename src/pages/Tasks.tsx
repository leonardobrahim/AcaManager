...
import { Plus, CheckCircle2, Circle, CalendarIcon, Trash2, Edit2, GraduationCap } from 'lucide-react';
import { format, isPast, isToday } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { cn } from '@/src/lib/utils';
import { TaskStatus, Priority, StudyStatus, Assignment, Exam } from '@/src/types';
...
export function Tasks() {
  ...
  const filteredAssignments = assignments.filter(a => selectedSubject === 'all' || a.subjectId === selectedSubject);
  const sortedAssignments = [...filteredAssignments].sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime());

  const filteredExams = exams.filter(e => selectedSubject === 'all' || e.subjectId === selectedSubject);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Tarefas e Provas</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Gerencie os seus prazos e avaliações.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex p-1 bg-slate-200/50 dark:bg-slate-800/50 rounded-xl">
            <button
              onClick={() => { setActiveTab('assignments'); resetForms(); }}
              className={cn("px-5 py-2 text-sm font-semibold rounded-lg transition-all duration-200", activeTab === 'assignments' ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200")}
            >
              Tarefas
            </button>
            <button
              onClick={() => { setActiveTab('exams'); resetForms(); }}
              className={cn("px-5 py-2 text-sm font-semibold rounded-lg transition-all duration-200", activeTab === 'exams' ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200")}
            >
              Provas
            </button>
          </div>

          <Button onClick={() => { resetForms(); setIsAdding(!isAdding); }} className="hidden sm:flex shadow-sm">
            <Plus className="mr-2 h-4 w-4" /> Nova {activeTab === 'assignments' ? 'Tarefa' : 'Prova'}
          </Button>
        </div>
      </div>

      <Button onClick={() => { resetForms(); setIsAdding(!isAdding); }} className="w-full sm:hidden shadow-sm">
        <Plus className="mr-2 h-4 w-4" /> Nova {activeTab === 'assignments' ? 'Tarefa' : 'Prova'}
      </Button>
          ...
          <CardContent className="p-6">
            ...
            <form onSubmit={handleSaveAssignment} className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Data de Entrega *</label>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <Button type="button" variant="ghost" onClick={resetForms}>Cancelar</Button>
                <Button type="submit">{editingAssignmentId ? 'Guardar Alterações' : 'Guardar Tarefa'}</Button>
              </div>
            </form>
          </CardContent>
        </Card>
        ...
        <Card className="border-indigo-100 dark:border-indigo-900/50 bg-indigo-50/30 dark:bg-indigo-900/10 shadow-sm">
          <CardContent className="p-6">
            ...
            <form onSubmit={handleSaveExam} className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Título da Prova *</label>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <Button type="button" variant="ghost" onClick={resetForms}>Cancelar</Button>
                <Button type="submit">{editingExamId ? 'Guardar Alterações' : 'Guardar Prova'}</Button>
              </div>
            </form>
          </CardContent>
        </Card>
      ...
      {activeTab === 'assignments' && sortedAssignments.length === 0 && !isAdding && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="text-xl font-semibold text-slate-900 dark:text-white">Nenhuma tarefa</h3>
          <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-sm">Tudo em dia! Adicione uma tarefa para acompanhá-la aqui.</p>
        </div>
      )}
      ...
      {activeTab === 'exams' && sortedExams.length === 0 && !isAdding && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="text-xl font-semibold text-slate-900 dark:text-white">Nenhuma prova</h3>
          <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-sm">Nenhuma prova próxima. Adicione uma para começar a preparar-se.</p>
        </div>
      )}
      ...
      {activeTab === 'assignments' && sortedAssignments.length > 0 && (
        <div className="space-y-3">
          {sortedAssignments.map((assignment) => {
            ...
            return (
              <Card key={assignment.id} className={cn("transition-all hover:shadow-md", isCompleted ? "opacity-60 bg-slate-50/50 dark:bg-slate-900/50" : "bg-white dark:bg-slate-900")}>
                <CardContent className="p-4 flex items-start sm:items-center gap-4">
                  <button
                    ...
                    className="mt-1 sm:mt-0 flex-shrink-0 text-slate-300 dark:text-slate-600 hover:text-emerald-500 dark:hover:text-emerald-400 transition-colors"
                  >
                    {isCompleted ? <CheckCircle2 className="h-6 w-6 text-emerald-500 dark:text-emerald-400" /> : <Circle className="h-6 w-6" />}
                  </button>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
                      <p className={cn("font-semibold text-slate-900 dark:text-white truncate", isCompleted && "line-through text-slate-500 dark:text-slate-400")}>
                        {assignment.title}
                      </p>
                      <div className="flex items-center gap-3 text-xs sm:text-sm whitespace-nowrap">
                        <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                          ...
                          <span className="font-medium text-slate-600 dark:text-slate-300">{subject?.name || 'Desconhecida'}</span>
                        </div>
                        <span className={cn("flex items-center gap-1.5 font-medium", isOverdue ? "text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded-md" : "text-slate-500 dark:text-slate-400")}>
                          <CalendarIcon className="h-3.5 w-3.5" />
                          {format(new Date(assignment.dueDate), "d 'de' MMM, yyyy", { locale: ptBR })}
                        </span>
                      </div>
                    </div>
                    {assignment.notes && <p className="text-sm text-slate-500 dark:text-slate-400 mt-1.5 truncate">{assignment.notes}</p>}
                  </div>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <Button variant="ghost" size="icon" onClick={() => handleEditAssignment(assignment)} className="text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20">
                      <Edit2 className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => deleteAssignment(assignment.id)} className="text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20">
      ...
      {activeTab === 'exams' && sortedExams.length > 0 && (
        <div className="space-y-3">
          {sortedExams.map((exam) => {
            ...
            return (
              <Card key={exam.id} className={cn("transition-all hover:shadow-md", isPastExam ? "opacity-60 bg-slate-50/50 dark:bg-slate-900/50" : "bg-white dark:bg-slate-900")}>
                <CardContent className="p-5 flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-bold text-lg text-slate-900 dark:text-white truncate">{exam.title}</h3>
                    ...
                    {exam.notes && <p className="text-sm text-slate-500 dark:text-slate-400 mt-3 bg-slate-50 dark:bg-slate-800/30 p-2 rounded-md">{exam.notes}</p>}
                  </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-2 mt-4 sm:mt-0 border-t dark:border-slate-800 sm:border-t-0 pt-4 sm:pt-0 border-slate-100">
                    <div className="flex flex-col gap-1.5 mr-2">
                      ...
                      <select
                        ...
                        className={cn(
                          ...
                          exam.studyStatus === 'ready' ? "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300" :
                          exam.studyStatus === 'studying' ? "bg-amber-100 dark:bg-amber-900/40 text-amber-800 dark:text-amber-300" :
                    ...
                    <Button variant="ghost" size="icon" onClick={() => handleEditExam(exam)} className="text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20">
                      <Edit2 className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => deleteExam(exam.id)} className="text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20">
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </div>
                </CardContent>
