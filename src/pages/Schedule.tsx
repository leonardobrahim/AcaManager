    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            A sua rotina semanal e calendário mensal.
          </p>
        </div>
      ...
      {activeTab === "weekly" && (
        <Card className="dark:bg-slate-900 dark:border-slate-800 shadow-sm overflow-x-auto">
          <CardContent className="p-0 min-w-200">
            <div className="grid grid-cols-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30">
              ...
              {DAYS_OF_WEEK.map((day) => (
                <div
                  key={day.id}
                  className="p-4 text-center font-semibold text-sm text-slate-700 dark:text-slate-300 border-r border-slate-100 dark:border-slate-800 last:border-0"
                >
                  {day.name}
                </div>
              ))}
                ...
                <div key={time} className="grid grid-cols-6">
                  ...
                  {DAYS_OF_WEEK.map((day) => {
                    ...
                    return (
                      <div
                        key={`${day.id}-${time}`}
                        className="p-1 min-h-15 border-r border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"
                      >
                        {slotData && (
                          <div
      ...
      {activeTab === "calendar" && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-sm">
            A visão de calendário em formato Google Calendar está a ser
            construída e estará disponível em breve!
          </p>
        </div>
      )}
    </div>
