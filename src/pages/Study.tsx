...
export function Study() {
  ...
  const filteredTopics = studyTopics.filter(
    (t) => selectedSubject === "all" || t.subjectId === selectedSubject,
  );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Organize o que precisa de rever.
          </p>
        </div>
          ...
          <CardContent className="p-6">
            ...
            <form onSubmit={handleSaveTopic} className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewTopic({ ...newTopic, subjectId: e.target.value })
                    }
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <Button type="submit">
                  {editingTopicId ? "Guardar Alterações" : "Guardar Tópico"}
                </Button>
              </div>
            </form>
          </CardContent>
      ...
      {sortedTopics.length === 0 && !isAdding ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="text-xl font-semibold text-slate-900 dark:text-white">
            Nenhum tópico de estudo
          </h3>
          <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-sm">
            Adicione tópicos para organizar as suas sessões de estudo e manter o
            foco.
          </p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <CardContent className="p-5 flex items-start gap-4">
                  <button
                    onClick={() =>
                      updateStudyTopic(topic.id, { completed: !isCompleted })
                    }
                  ...
                  >
                    {isCompleted ? (
                      <CheckSquare className="h-6 w-6 text-emerald-500 dark:text-emerald-400" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {topic.notes && (
                      <p className="text-sm text-slate-500 dark:text-slate-400 mt-3 bg-slate-50 dark:bg-slate-800/30 p-2 rounded-md truncate">
                        {topic.notes}
                      </p>
                    )}
                  </div>
                  <div className="flex items-center flex-col sm:flex-row gap-1 -mr-2 -mt-2">
                    <Button
                      ...
                      className="text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 h-8 w-8"
                    >
                    ...
                    <Button
                      ...
                      className="text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 h-8 w-8"
                    >
